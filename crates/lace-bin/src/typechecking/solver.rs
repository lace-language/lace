...
impl<'a> Solver<'a> {
    ...
    pub fn apply_constraints(&mut self, constraints: Vec<Constraint>) {
        ...
        let mut uf = self.uf.take().unwrap_or_lice("don't call apply_constraints while applying constraints!");
    ...
    pub fn generate_type_errors(self, name_mapping: NameMapping) -> Result<SolvedTypes<'a>, TypeError> {
        ...
        Ok(SolvedTypes {
            ...
            uf: self.uf.unwrap_or_lice("don't generate type errors while constraint processing"),
        })
    }
...
impl<'a> SolvedTypes<'a> {
    ...
    fn find_representative(&self, var: TypeVariable) -> TypeVariable {
        TypeVariable::from_usize(self.uf.find(&var.as_usize()).unwrap_or_lice("type variable not in union find"))
    }
